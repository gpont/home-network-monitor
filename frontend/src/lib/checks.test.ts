...
describe("gw_mtu 1492 threshold", () => {
  const iface = { interfaceName: "en0", status: "up" as const, ipv4: "192.168.1.254", ipv6LinkLocal: null, gatewayIp: "192.168.1.1", gatewayMac: "aa:bb:cc:dd:ee:ff", connectionType: "dhcp" as const, rxErrors: 0, txErrors: 0, rxDropped: 0, txDropped: 0, timestamp: Date.now() };

  const mtuCheck = CHECKS.find(c => c.id === "gw_mtu")!;
  const pathMtuCheck = CHECKS.find(c => c.id === "path_mtu")!;

  test("gw_mtu: ok for MTU 1492 (PPPoE router path, DHCP client)", () => {
    const s = { ...emptyStatus(), interface: iface, mtu: { status: "fragmentation_detected", value: JSON.stringify({ maxMtu: 1492 }), timestamp: Date.now() } };
    expect(mtuCheck.getStatus(s as any)).toBe("ok");
  });
  test("gw_mtu: ok for MTU 1500", () => {
    const s = { ...emptyStatus(), interface: iface, mtu: { status: "ok", value: JSON.stringify({ maxMtu: 1500 }), timestamp: Date.now() } };
    expect(mtuCheck.getStatus(s as any)).toBe("ok");
  });
  test("gw_mtu: warn for MTU 1460 (below 1492)", () => {
    const s = { ...emptyStatus(), interface: iface, mtu: { status: "fragmentation_detected", value: JSON.stringify({ maxMtu: 1460 }), timestamp: Date.now() } };
    expect(mtuCheck.getStatus(s as any)).toBe("warn");
  });
  test("gw_mtu: no fix shown for MTU 1492", () => {
    const s = { ...emptyStatus(), interface: iface, mtu: { status: "fragmentation_detected", value: JSON.stringify({ maxMtu: 1492 }), timestamp: Date.now() } };
    expect(mtuCheck.getFix(s as any)).toBeNull();
  });
  test("path_mtu: ok for MTU 1492", () => {
    const s = { ...emptyStatus(), interface: iface, mtu: { status: "fragmentation_detected", value: JSON.stringify({ maxMtu: 1492 }), timestamp: Date.now() } };
    expect(pathMtuCheck.getStatus(s as any)).toBe("ok");
  });
  test("path_mtu: warn for MTU 1400 (below 1492, VPN overhead)", () => {
    const s = { ...emptyStatus(), interface: iface, mtu: { status: "fragmentation_detected", value: JSON.stringify({ maxMtu: 1400 }), timestamp: Date.now() } };
    expect(pathMtuCheck.getStatus(s as any)).toBe("warn");
  });
});
