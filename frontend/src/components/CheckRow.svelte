<script lang="ts">
  ...
  const statusLabel: Record<CheckStatus, string> = {
    ok: 'OK', fail: 'FAIL', warn: 'WARN', stale: 'устарело', unknown: 'нет данных', info: 'INFO',
  };
...
<div style="padding: 5px 8px; border-radius: 6px; background: {bg}; margin-bottom: 1px;">
  <div style="display: flex; align-items: flex-start; gap: 8px;">
    <span
      title="{statusLabel[st]}: {check.description}"
      style="
        display: inline-flex; align-items: center; justify-content: center;
        width: 18px; height: 18px; border-radius: 50%;
        background: {color}; color: white; font-size: 10px; font-weight: bold;
        flex-shrink: 0; margin-top: 2px; cursor: default;
      ">{icon}</span>
    <div style="flex: 1; min-width: 0;">
      <div style="display: flex; align-items: baseline; gap: 6px; flex-wrap: wrap;">
        <span
          title="{check.description}"
          style="font-weight: 500; font-size: 13px; color: {isNoData ? '#4b5563' : '#e2e8f0'}; cursor: default;"
        >{check.name}</span>
        {#if val && !isNoData}
        ...
        {#if isNoData}
          <span style="font-size: 11px; color: #4b5563; font-style: italic;">
            {st === 'stale' ? 'данные устарели' : 'нет данных'}
          </span>
        {/if}
      </div>
      {#if !isNoData}
        <div style="font-size: 11px; color: #475569; margin-top: 1px;">{check.description}</div>
      {/if}
      {#if fix && fix.length > 0}
        <div style="margin-top: 5px; padding: 5px 8px; background: rgba(239,68,68,0.1); border-left: 2px solid #ef4444; border-radius: 3px;">
